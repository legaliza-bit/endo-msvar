...
from jax.scipy import linalg, stats
...
def _calc_tvtp(state_exog, state_coeffs):
    ...
    tvtp = stats.norm.cdf(
        np.kron(
            np.matmul(state_exog, state_coeffs).reshape(T, 1, 2), np.array([[1], [-1]])
        )
    )
...
def calc_endo_tp(state_exog, state_coeffs, resids, sigmas, rho):
    ...
    endo_tp = stats.norm.cdf(
        (
            ...
            - rho
            * np.kron(
                np.moveaxis(resids.reshape(T, 1, 2) / sigmas.reshape(1, 2), -1, -2),
                np.array([[1, 1]]),
            )
        )
        / np.sqrt(1 - rho**2)
...
def _calc_cond_density(resids, sigmas, trans_probs, endo_tp):
    ...
    """
    exog_part = stats.norm.pdf(
        resids.reshape(2, 1) / sigmas.reshape(2, 1)
    ) / sigmas.reshape(2, 1)
...
def hamilton_filter(params, endog, exog, state_exog, exog_switching=False):
    ...
    endo_tp = calc_endo_tp(state_exog, state_coeffs, resids, sigmas, rho)
...
def obj_func(params, endog, exog, state_exog, exog_switching=False):
    total_likelihood, _, _ = hamilton_filter(
        params, endog, exog, state_exog, exog_switching
    )
    return -total_likelihood


@jax.jit
def sample_states(key, tvtp, filter):
