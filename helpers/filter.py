...
def initialize_tp(z):
    ...
    z - vector
    """
    lindex = alpha + beta * np.mean(z)
    return stats.norm.cdf(lindex)

def reshape(ts, order):
    pass

def calc_endo_tp(z, em, alpha, beta, rho, S):
    lindex1 = alpha + z.T @ beta - rho * (em[0] / S[0])
    lindex2 = - (alpha + z.T @ beta) + rho * (em[1] / S[1])
    denom = np.sqrt(1 - rho**2)
    return np.array([stats.norm.cdf(lindex1 / denom),
                     stats.norm.cdf(lindex2 / denom)])

def calc_tvtp(z, alpha, beta):
    lindex = alpha + z.T @ beta
    return np.array([stats.norm.cdf(lindex),
                     1 - stats.norm.cdf(lindex)])


def hamFilt(x,y,z,S1,S2,beta0,beta1,S,mu,rho):
    """
    ...
    trans_probs = _regime_transition_matrix_tvtp()

    for i in range(t):
        # Get state means for prev num_lags states
        if i >= num_lags:
           prev_mu = [mu[idx] for idx in S[i-num_lags:i]]
        else:
            [mu[idx] for idx in S[:i]] 

        # Pad in case too short
        if len(prev_mu) < num_lags:
            prev_mu = [0]*(num_lags-len(prev_mu)) + prev_mu

        # Add [0] for the constant
        prev_mu = prev_mu + [0]

        # First, compute likelihood of y_i given the past states (g(y_i|S_i))
        em1 = (y[i] - mu[0]) - ((x.iloc[i,].values - prev_mu) @ beta0)  # Compute residual
        em2 = (y[i] - mu[1]) - ((x.iloc[i,].values - prev_mu) @ beta1)  # Compute residual

        endo_tp = calc_endo_tp(z, (em1, em2), alpha, beta, rho, (S1, S2))
        tvtp = trans_probs[i:,]
        neta1 = endo_tp / (tvtp * S1) * (1/np.sqrt(2*math.pi*S1)) * np.exp((-1/(2*S1)) * (em1**2))  # Gaussian likelihood
        neta2 = endo_tp / (tvtp * S2) * (1/np.sqrt(2*math.pi*S2)) * np.exp((-1/(2*S2)) * (em2**2))  # Gaussian likelihood
