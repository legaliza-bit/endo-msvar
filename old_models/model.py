...
class EMSAR:
    ...
    def _init_params(self, n_feat, n_state_feat, coeffs=None, state_coeffs=None, rho=None, sigmas=None):
        coeffs = (
            np.random.uniform(low=-0.99, high=0.99, size=(n_feat, 2))
            if not coeffs
            else coeffs
        )
        state_coeffs = (
            np.random.uniform(low=-0.99, high=0.99, size=(n_state_feat, 2))
            if not state_coeffs
            else state_coeffs
        )
        rho = np.array(np.random.uniform(low=-0.99, high=0.99)) if not rho else rho
        sigmas = np.random.uniform(low=0.01, high=1, size=2) if not sigmas else sigmas
        return {"coeffs": coeffs, "state_coeffs": state_coeffs,
                "rho": rho, "sigmas": sigmas}
    ...
    def calc_tvtp(self, state_exog, state_coeffs):
        ...
        assert (np.sum(tvtp, axis=2) == 1).all(), np.sum(tvtp, axis=2)
        return tvtp
    ...
    def sample_states(self):
        ...
        p2 = filter[-1, 1]
        p = p1 / (p1 + p2)
        u = np.random.uniform(0, 1)  # Unif(0,1), draw 1
