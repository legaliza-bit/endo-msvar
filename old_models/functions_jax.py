...
def calc_tvtp(state_exog, state_coeffs):
    ...
    tvtp = np.stack(
        [
            # shape (T, 2)
            _calc_tvtp(state_exog, state_coeffs[:, 0]),
            _calc_tvtp(state_exog, state_coeffs[:, 1]),
        ]
    )
...
@jax.jit
def _calc_endo_tp(state_exog, resid, state_coeffs, sigma, rho):
    """
    For 1 state only.
    """
    lindex = (
...
def calc_endo_tp(state_exog, resids, sigmas, state_coeffs, rho):
    ...
    endo_tp = np.stack(
        [
            # shape (1, 2)
            _calc_endo_tp(state_exog, resids[0], state_coeffs[:, 0], sigmas[0], rho),
            _calc_endo_tp(state_exog, resids[1], state_coeffs[:, 1], sigmas[1], rho),
        ]
    )
...
@jax.jit
def _calc_cond_density(resids, sigmas, trans_probs, endo_tp=None):
    """
...
@jax.jit
def calc_cond_llhs(resids, sigmas, trans_probs, state_probs, endo_tp=None):
    """
...
def hamilton_filter(params, endog, exog, state_exog):
    ...
    for i in range(T):
        ...
        endo_tp = calc_endo_tp(state_exog[i, :], resids, sigmas, state_coeffs, rho)

        # f(y_t | S_t=i, S_{t-1}=j, Z_t, Y_{t-1}; params)
        cond_llhs = calc_cond_llhs(resids, sigmas, tp, state_probs, endo_tp)
